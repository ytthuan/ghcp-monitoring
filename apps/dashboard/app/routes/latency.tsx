import { createFileRoute } from "@tanstack/react-router";
import { createServerFn } from "@tanstack/react-start";
import { useQuery } from "@tanstack/react-query";
import { Gauge } from "lucide-react";
import { useFilters } from "~/lib/use-filters";
import { ChartSkeleton } from "~/components/layout/Skeletons";
import { EmptyState } from "~/components/layout/EmptyState";
import { FormatCell } from "~/components/data/FormatCell";
import {
  Table,
  TableBody,
  TableCell,
  TableHead,
  TableHeader,
  TableRow,
} from "~/components/ui/table";
import { FiltersSchema, type Filters } from "~/lib/types";
import { formatMs, formatNumber } from "~/lib/format";
import {
  ChartCard,
  LegendList,
  QuickStat,
  SidePanelSection,
  type LegendItem,
} from "./-charts/ChartCard";

const fetchLatency = createServerFn({ method: "POST" })
  .inputValidator((d: Filters) => FiltersSchema.parse(d))
  .handler(async ({ data }) => {
    const { getLatencyByModel } = await import("~/server/queries/latency");
    return getLatencyByModel(data);
  });

export const Route = createFileRoute("/latency")({
  component: LatencyPage,
});

function LatencyPage() {
  const { filters } = useFilters();
  const q = useQuery({
    queryKey: ["latency", filters],
    queryFn: () => fetchLatency({ data: filters }),
  });

  if (q.isLoading) return <ChartSkeleton height={400} />;
  if (q.error) throw q.error;
  if (!q.data || q.data.length === 0) {
    return (
      <EmptyState
        icon={Gauge}
        title="No latency data in this range"
        description="Try widening the time range or removing model filters."
      />
    );
  }

  let totalCount = 0;
  let p50Sum = 0;
  let p90Sum = 0;
  let p99Sum = 0;
  let slowestModel = q.data[0]?.model ?? "—";
  let slowestP90 = 0;
  for (const r of q.data) {
    totalCount += r.count;
    p50Sum += r.p50_ms * r.count;
    p90Sum += r.p90_ms * r.count;
    p99Sum += r.p99_ms * r.count;
    if (r.p90_ms > slowestP90) {
      slowestP90 = r.p90_ms;
      slowestModel = r.model;
    }
  }
  const aggP50 = totalCount > 0 ? p50Sum / totalCount : 0;
  const aggP90 = totalCount > 0 ? p90Sum / totalCount : 0;
  const aggP99 = totalCount > 0 ? p99Sum / totalCount : 0;

  const callsByModel: LegendItem[] = q.data
    .map((r) => ({
      key: r.model,
      label: r.model,
      value: r.count,
      share: totalCount > 0 ? r.count / totalCount : 0,
    }))
    .sort((a, b) => b.value - a.value);

  return (
    <ChartCard
      eyebrow="End-to-end chat latency distribution"
      title="Latency by model"
      whatThisMeasures="Per-model wall-clock duration of chat calls computed from otel_traces.Duration. Lower is better."
      stat={formatMs(aggP50)}
      statLabel={`Median latency across ${formatNumber(totalCount)} calls`}
      drillTo="/calls"
      chart={
        <>
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>Model</TableHead>
                <TableHead className="text-right">Calls</TableHead>
                <TableHead className="text-right">p50</TableHead>
                <TableHead className="text-right">p90</TableHead>
                <TableHead className="text-right">p99</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {q.data.map((r) => (
                <TableRow key={r.model}>
                  <TableCell>{r.model}</TableCell>
                  <TableCell className="text-right">
                    <FormatCell kind="number" value={r.count} />
                  </TableCell>
                  <TableCell className="text-right">
                    <FormatCell kind="ms" value={r.p50_ms} />
                  </TableCell>
                  <TableCell className="text-right">
                    <FormatCell kind="ms" value={r.p90_ms} />
                  </TableCell>
                  <TableCell className="text-right">
                    <FormatCell kind="ms" value={r.p99_ms} />
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
          <p className="mt-3 text-xs text-muted-foreground">
            Quantiles computed from <code>otel_traces.Duration</code>. To use the
            histogram metric instead, configure{" "}
            <code>gen_ai.client.operation.duration</code> in the collector.
          </p>
        </>
      }
      side={
        <>
          <SidePanelSection heading="Aggregate quantiles">
            <div className="space-y-1.5">
              <QuickStat label="p50" value={formatMs(aggP50)} />
              <QuickStat label="p90" value={formatMs(aggP90)} />
              <QuickStat label="p99" value={formatMs(aggP99)} />
              <QuickStat
                label="Slowest model (p90)"
                value={slowestModel}
                hint={`${slowestModel} has the highest p90 (${formatMs(slowestP90)}).`}
              />
            </div>
          </SidePanelSection>
          <SidePanelSection heading="Calls by model">
            <LegendList items={callsByModel} formatValue={formatNumber} />
          </SidePanelSection>
        </>
      }
    />
  );
}
